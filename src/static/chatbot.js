(function () {
  ...
  style.textContent = `
    #chatbot-container {
      position: fixed; bottom: 20px; right: 20px; width: 700px; min-height:600px;
      font-family: sans-serif; background: white; border-radius: 10px;
      box-shadow: 0 0 10px rgba(0,0,0,0.2); padding: 10px; z-index: 9999;
    }
    #chatbot-messages { max-height: 600px; overflow-y: auto; font-size: 14px; }
  ...
  function setupEventListeners() {
    ...
    input.addEventListener("keypress", async (e) => {
      if (e.key === "Enter" && input.value.trim()) {
        ...
        if (data.response) {
          messages.innerHTML += `<div><b>Bot:</b> ${converter.makeHtml(data.response)}</div>`;
        } else {
